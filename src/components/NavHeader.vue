          ...
          <div class="confirm-tips">
            ...
            <ul>
              <li class="regi_form_input">
                ...
                <input class="regi_login_input regi_login_input_left" 
                       v-model="userName" type="text" tabindex="1" name="loginname" 
                       placeholder="User Name" data-type="loginname">
              </li>
              <li class="regi_form_input noMargin">
                <i class="icon IconPwd"></i>
                <input class="regi_login_input regi_login_input_left login-input-no input_text"
                       v-model="userPwd"  type="password" tabindex="2"  name="password" 
      ...
      methods:{
        login() {// 用户登录
          ...
          axios.post('/users/login',{
            ...
            if(res.status == '0'){// 登录成功
              ...
              this.$store.commit("updateUserInfo",res.result.userName);// 将用户名提交到Vuex
              this.getCartCount();// 登录成功获取购物车商品数量
           }
            else{// 登录失败
              this.errorTip = true;
            }
        ...
        logOut() {// 用户登出
          axios.post('/users/logout').then((response)=>{
            ...
            if(res.status == '0'){// 登出成功
              // this.nickName = '';
              this.getCartCount();
              this.$store.commit("updateUserInfo",res.result.userName);
              if(this.$route.path!="/"){// 如退出登录不是在主页，跳转到主页
        ...
        checkLogin(){// 登录校验
          axios.get('/users/checkLogin').then((response)=>{
            ...
            if(res.status == '0'){
              ...
              if(this.$route.path!="/"){// 如果当前不是在主页，跳转到主页
                this.$router.push({ path: '/' });
              }
            }
          })
