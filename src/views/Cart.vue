...
<script>
    ...
    export default{
        name:'Cart',
        components:{NavHeader,NavFooter,NacBread,Modal},
        data() {
            return {
                cartList:[],
                productId:'',
        ...
        methods:{
          ...
          delCart(){
            axios.post('/users/cartDel',{
              ...
              // 如果删除成功，关闭模态窗->重新加载购物车列表
              if(res.status == '0'){
                this.modalConfirm = false;
                let delCount = this.delItem.productNum;// 删除的商品数量
          ...
          // 购物车删除按钮
          delCartConfirm(item){
            // this.productId = productId // 获取要删除商品的商品ID
            this.delItem = item;
            this.modalConfirm = true;  // 模态框打开
          },
          // 商品增加删除
          editCart(flag,item) {
            if(flag == 'add'){// 商品增加
